...
class CollectionsTest: XCTestCase, WrapperTestCallback {
    ...
    func testSingleCollection() {
        let params = Parameters()
            .add(ids: "1194")
        
        wrapper?.collections(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            let result = jsonResponse[0]
            
            let charId = result["id"] as? Int
            
            XCTAssertEqual(1194, charId)
    ...
    func testMultipleCollection() {
        let params = Parameters()
            .add(ids: "1198,1199,1200")
        ...
        wrapper?.collections(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            ...
            XCTAssertEqual(id1, 1198)
            XCTAssertEqual(id2, 1199)
            XCTAssertEqual(id3, 1200)
            self.testCallback?.onDone(results: "finished")
