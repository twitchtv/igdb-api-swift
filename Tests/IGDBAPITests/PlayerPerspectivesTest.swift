...
class PlayerPerspectivesTest: XCTestCase, WrapperTestCallback {
    ...
    func testSinglePlayerPerspective(){
        let params = Parameters()
            .add(ids: "2")
        
        
        wrapper?.playerPerspectives(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
    ...
    func testMultipleCharacters(){
        ...
        wrapper?.playerPerspectives(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            ...
            XCTAssertEqual(id2, 5)
            XCTAssertEqual(id3, 4)
            self.testCallback?.onDone(results: "finished")
