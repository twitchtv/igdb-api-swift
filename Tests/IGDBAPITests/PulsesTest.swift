...
class PulsesTest: XCTestCase, WrapperTestCallback {
    ...
    func testSinglePulses(){
        let params = Parameters()
            .add(ids: "24000")
        
        
        wrapper?.pulses(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            let result = jsonResponse[0]
            
            let id = result["id"] as? Int
            
            XCTAssertEqual(24000, id)
    ...
    func testMultiplePulses(){
        let params = Parameters()
            .add(ids: "25040,25047,25000")
        ...
        wrapper?.pulses(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            ...
            XCTAssertEqual(id1, 25040)
            XCTAssertEqual(id2, 25047)
            XCTAssertEqual(id3, 25000)
            self.testCallback?.onDone(results: "finished")
