...
class PlatformsTest: XCTestCase, WrapperTestCallback {
    ...
    func testSinglePlatform(){
        let params = Parameters()
            .add(ids: "11")
        
        
        wrapper?.platforms(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            let result = jsonResponse[0]
            
            let id = result["id"] as? Int
            
            XCTAssertEqual(11, id)
    ...
    func testPlatformsSearchMultiAndSingle(){
        let params = Parameters()
            .add(search: "xbox one")
            .add(fields: "name");
        ...
        wrapper?.search(endpoint: .PLATFORMS, params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            ...
            XCTAssertEqual(id, 49)
            XCTAssertEqual(name, "Xbox One")
            self.testCallback?.onDone(results: "finished")
