...
class FranchisesTest: XCTestCase, WrapperTestCallback {
    ...
    func testSingleFranchises(){
        let params = Parameters()
            .add(ids: "1040")
        
        
        wrapper?.franchises(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            let result = jsonResponse[0]
            
            let id = result["id"] as? Int
            
            XCTAssertEqual(1040, id)
