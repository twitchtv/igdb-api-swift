...
class FeedTest: XCTestCase, WrapperTestCallback {
    ...
    func testSingleFeed(){
        ...
        wrapper?.feeds(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            let result = jsonResponse[0]
            
            let id = result["id"] as? Int
            
            XCTAssertEqual(129989, id)
    ...
    func testMultipleFeed(){
        let params = Parameters()
            .add(ids: "129993,129992,129985")
        ...
        wrapper?.feeds(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            ...
            XCTAssertEqual(id1, 129993)
            XCTAssertEqual(id2, 129992)
            XCTAssertEqual(id3, 129985)
