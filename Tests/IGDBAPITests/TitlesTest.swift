...
class TitlesTest: XCTestCase, WrapperTestCallback {
    ...
    func testSingleTitle(){
        ...
        wrapper?.titles(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            let result = jsonResponse[0]
            
            let id = result["id"] as? Int
            
            XCTAssertEqual(29, id)
    ...
    func testMultipleThemes(){
        ...
        wrapper?.titles(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            ...
            XCTAssertEqual(id1, 29)
            XCTAssertEqual(id2, 14)
            XCTAssertEqual(id3, 22)
            self.testCallback?.onDone(results: "finished")
