...
class CreditsTest: XCTestCase, WrapperTestCallback {
    ...
    func testSingleCredits(){
        let params = Parameters()
            .add(ids: "1073917668")
        
        
        wrapper?.credits(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            let result = jsonResponse[0]
            
            let id = result["id"] as? Int
            
            XCTAssertEqual(1073917668, id)
    ...
    func testMultipleCredits(){
        let params = Parameters()
            .add(ids: "1073917668,1073917629,1073917617")
        ...
        wrapper?.credits(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            ...
            XCTAssertEqual(id1, 1073917668)
            XCTAssertEqual(id2, 1073917629)
            XCTAssertEqual(id3, 1073917617)
