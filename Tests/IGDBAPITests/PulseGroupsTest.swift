...
class PulseGroupsTest: XCTestCase, WrapperTestCallback {
    ...
    func testSinglePulseGroup(){
        let params = Parameters()
            .add(ids: "5768")
        
        
        wrapper?.pulseGroups(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            let result = jsonResponse[0]
            
            let id = result["id"] as? Int
            
            XCTAssertEqual(5768, id)
    ...
    func testMultiplePulseGroups(){
        let params = Parameters()
            .add(ids: "5772,5775,5777")
        ...
        wrapper?.pulseGroups(params: params, onSuccess: {(jsonResponse: [[String: AnyObject]]) -> (Void) in
            ...
            XCTAssertEqual(id1, 5772)
            XCTAssertEqual(id2, 5775)
            XCTAssertEqual(id3, 5777)
            self.testCallback?.onDone(results: "finished")
